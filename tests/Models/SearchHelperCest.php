...
final class SearchHelperCest
{
    ...
    public function testContentQueryDefaultUsesEntityScope(ModelsTester $I): void
    {
        $this->ensureSchemas();

        $visible = new Content();
        $visible->setName('Aiguillage visible page');
        $visible->setActive(true);
        $visible->save();

        $hidden = new Content();
        $hidden->setName('Aiguillage hidden page');
        $hidden->setActive(false);
        $hidden->save();

        $entityResults = SearchHelper::contentQuery('Aiguillage')->all();
        $entityIds = array_map(fn($r) => $r->getId(), $entityResults);
        $I->assertContains($visible->getId(), $entityIds);
        $I->assertNotContains($hidden->getId(), $entityIds);

        $modelResults = SearchHelper::contentQuery('Aiguillage', Content::class)->all();
        $modelIds = array_map(fn($r) => $r->getId(), $modelResults);
        $I->assertContains($visible->getId(), $modelIds);
        $I->assertContains($hidden->getId(), $modelIds);
    }
    ...
    public function testTagQueryDefaultUsesEntityScope(ModelsTester $I): void
    {
        $this->ensureSchemas();

        $visible = new Tag();
        $visible->setName('TagSwitch visible');
        $visible->setActive(true);
        $visible->save();

        $hidden = new Tag();
        $hidden->setName('TagSwitch hidden');
        $hidden->setActive(false);
        $hidden->save();

        $entityResults = SearchHelper::tagQuery('TagSwitch')->all();
        $entityIds = array_map(fn($r) => $r->getId(), $entityResults);
        $I->assertContains($visible->getId(), $entityIds);
        $I->assertNotContains($hidden->getId(), $entityIds);

        $modelResults = SearchHelper::tagQuery('TagSwitch', Tag::class)->all();
        $modelIds = array_map(fn($r) => $r->getId(), $modelResults);
        $I->assertContains($visible->getId(), $modelIds);
        $I->assertContains($hidden->getId(), $modelIds);
    }
