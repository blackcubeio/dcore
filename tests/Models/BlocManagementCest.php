...
final class BlocManagementCest
{
    ...
    public function testMoveBlocToNewPosition(ModelsTester $I): void
    {
        ...
        $I->assertEquals($bloc3->getId(), $pivots[1]->getBlocId());
        $I->assertEquals($bloc1->getId(), $pivots[2]->getBlocId());
    }
    ...
    public function testReorderBlocs(ModelsTester $I): void
    {
        ...
        $I->assertEquals($bloc3->getId(), $pivots[2]->getBlocId());
    }
    ...
    public function testXeoAttachBloc(ModelsTester $I): void
    {
        $xeo = $this->createXeo();
        $bloc1 = $this->createBloc();
        $bloc2 = $this->createBloc();

        $xeo->attachBloc($bloc1);
        $xeo->attachBloc($bloc2);

        $pivots = XeoBloc::query()
            ->andWhere(['xeoId' => $xeo->getId()])
            ->orderBy(['order' => SORT_ASC])
            ->all();
    ...
    public function testXeoDetachBloc(ModelsTester $I): void
    {
        $xeo = $this->createXeo();
        $bloc = $this->createBloc();
        $blocId = $bloc->getId();
        ...
        $pivot = XeoBloc::query()
            ->andWhere(['xeoId' => $xeo->getId(), 'blocId' => $blocId])
            ->one();
        $I->assertNull($pivot);
    ...
    public function testXeoMoveBloc(ModelsTester $I): void
    {
        $xeo = $this->createXeo();
        $bloc1 = $this->createBloc();
        $bloc2 = $this->createBloc();
        ...
        $xeo->attachBloc($bloc1);
        $xeo->attachBloc($bloc2);
        $xeo->attachBloc($bloc3);

        $xeo->moveBloc($bloc3, 1);
    ...
    private function createXeo(): Xeo
    {
        $slug = new Slug();
        $slug->setPath('/xeo-bloc-test-'.uniqid());
        $slug->save();
