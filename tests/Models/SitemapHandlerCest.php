...
final class SitemapHandlerCest
{
    ...
    {
        $result = $this->ensureService()->generate($this->wildcardHost(), 'localhost', 'https');
        $I->assertNull($result);
    }
    ...
    {
        $this->createTagWithSlug('my-category', 'My Category');

        $result = $this->ensureService()->generate($this->wildcardHost(), 'localhost', 'https');

        $I->assertNotNull($result);
        $I->assertStringContainsString('my-category', $result);
    }
