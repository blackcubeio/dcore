...
```php
$link = $content->getLink();              // ?Link (PSR-13, via LinkTrait)
$link = $tag->getLink();                  // ?Link (PSR-13, via LinkTrait)
```
...
### Resolution by element

```php
HandlerDescriptor::fromElement(Content|Tag $element): ?self
```

Resolution by path (host match, active Slug lookup, CMS element, Type handler) happens upstream in the SSR middleware, which then builds the descriptor from the element.
...
```php
HandlerDescriptor::fromError(string $route, string $languageId): ?self
```
...
```php
HandlerDescriptor::setRouteLoader(Closure $loader): void
```
