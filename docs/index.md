...
| Queries | `Model::query()` returns a `ScopableQuery` |
...
| `LlmMenu` | `llmMenus` | llms.txt navigation entry. Tree-structured (3 levels), references a Content or Tag. |
...
| `ContentTranslationGroup` | `contentTranslationGroups` | Translation group linking Contents together. |
| `TagTranslationGroup` | `tagTranslationGroups` | Translation group linking Tags together. |
...
|---|---|---|
| `#[Exportable]` | Property, Method | Marks a property/method for JSON export. Supports `name`, `format`, `fields`, `property`, `method`, `description`. |
