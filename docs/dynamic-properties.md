...
| `getElasticSchemaId()` (models) | `mixed` | Associated schema ID — defined on the models, not on the interface |
| `setElasticSchemaId(mixed $id)` (models) | `void` | Change the schema — defined on the models, not on the interface |
