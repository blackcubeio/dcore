...
class SearchHelper
{
        ...
        if ($cache === null) {
            ...
            foreach ($schemasQuery->each() as $schema) {
                ...
                foreach ($parsed['properties'] ?? [] as $name => $def) {
                    ...
                    if (($def['type'] ?? '') === 'string'
                        && ($format === null || in_array($format, ['textarea', 'wysiwyg'], true) === true)) {
     ...
     *
     * Results are ordered by total occurrence count (DESC). No pagination is applied.
     * Publishability follows $modelClass: Entities apply the publishable scope,
     * Models return every row.
     *
     * @param string $modelClass Content class to query (Entity by default = published only)
     */
    public static function contentQuery(string $search, string $modelClass = Content::class): ActiveQueryInterface
    {
        ...
        return $query
            ...
            ->having(['>', 'relevance', 0])
            ->orderBy(new Expression('relevance DESC'));
    }
     ...
     * Models return every row.
     *
     * @param string $modelClass Tag class to query (Entity by default = published only)
     */
    public static function tagQuery(string $search, string $modelClass = Tag::class): ActiveQueryInterface
    {
        $pageFields = self::getPageTextFields();
        $blocFields = self::getBlocTextFields();
        $occurrences = self::occurrenceCallback();
