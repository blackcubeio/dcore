...
abstract class BaseSitemap extends ActiveRecord
{
    ...
    #[Exportable(description: 'sitemap changefreq (e.g. daily, weekly).')]
    public function getFrequency(): string
    {
        return $this->frequency;
    }

    #[Importable(name: 'frequency')]
    public function setFrequency(string $frequency): void
    {
        $this->frequency = $frequency;
    }

    #[Exportable(description: 'sitemap priority (0.0 .. 1.0).')]
    public function getPriority(): float
    {
        return $this->priority;
    }

    #[Importable(name: 'priority')]
    public function setPriority(float $priority): void
    {
        $this->priority = $priority;
    }

    #[Exportable(description: 'Included in the sitemap when true.')]
    ...
    {
        return $this->hasOne($this->fqcn(Slug::class), ['id' => 'slugId']); // inverseOf 'sitemap' removed: yii RelationPopulator bug on nullable hasOne in single eager-load (with-each)
    }
}
